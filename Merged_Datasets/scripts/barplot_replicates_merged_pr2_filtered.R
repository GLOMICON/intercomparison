...
#figures
plot_dir <- './Merged_Datasets/figures/barplot_replicates_merged_pr2_filtered/'
...
for (val in markers) {
  ...
  # limit otu table
  merged_potu.c <- potu.c %>%
    full_join(meta_tab) %>%
    group_by(Analyzing_Institute, Collecting_Institute, site, ASV) %>%
    mutate(per_tot = mean(per_tot)) %>%
    mutate(reads = mean(reads)) %>%
    ungroup() %>%
    distinct(Analyzing_Institute, Collecting_Institute, site, ASV, reads, per_tot) %>%
    unite(SampleID, Analyzing_Institute, site, remove=FALSE )

  
  # limit metadata table:
  merged_meta_lim <- meta_tab %>%
    distinct(Analyzing_Institute, Collecting_Institute, site) %>%
    drop_na() %>%
    unite(SampleID, Analyzing_Institute, site, remove=FALSE )
  ...
  for (val in taxas) {
    top_20taxa <- make_top20_taxa(val, merged_potu.c, species_label, merged_meta_lim, sites)
    df <- merge_data_limit_byTopTaxa(top_20taxa, merged_potu.c, species_label, merged_meta_lim, sites)
  ...
  taxas <- c('division', 'class_join', 'order_join', 'family_join', 'genus_join', 'species_join')
  for (val in taxas) {
    top_20taxa <- make_top20_taxa(val, merged_potu.c, species_label, merged_meta_lim, controls)
    df <- merge_data_limit_byTopTaxa(top_20taxa, merged_potu.c, species_label, merged_meta_lim, controls)
  ...
  for (taxon in phyla) {
    ...
    top_20taxa <- make_top20_taxa('family_join', merged_potu.c, phy_lim, merged_meta_lim, sites)
    df <- merge_data_limit_byTopTaxa(top_20taxa, merged_potu.c, phy_lim, merged_meta_lim, sites)
